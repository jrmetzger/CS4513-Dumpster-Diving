...
void getSrcFilePath(char* file, char* dumpster_path, char** srcPath);
// Remove a folder.
...
int main(int argc, char** argv)
{
	...
	int sRtn = stat(dumpster_path, &dumpStat);
	if(sRtn)
	{
	...
	for(i = 0; i < fileCnt; i++)
	{
		...
		char* srcPath;
		getSrcFilePath(file, dumpster_path, &srcPath);
